...
class AttentionLayer(Layer):
  ...
  def call(self, inputs, mask=None):
    ...
    Start
    """
    #Attention layer is formed using the decoder outputs
    decoder_outputs_final = K.permute_dimensions(decoder_outputs, pattern=(0, 2, 1))
    #luong score is calculated with the final decoder outputs and encoder outputs with the softmax activation
    luong_score = K.expand_dims(K.softmax(K.batch_dot(encoder_outputs, decoder_outputs_final), axis=1))
    #finally the encoder vector are formed
...
class NmtModel(object):
  ...
  def build(self):
    ...
    source_embeddings = Embedding(input_dim = self.vocab_source_size ,output_dim = self.embedding_size, mask_zero = True)(source_words)
    target_embeddings = Embedding(input_dim = self.vocab_target_size ,output_dim = self.embedding_size ,mask_zero = True)(target_words)
    ...
    source_word_embeddings =  Dropout(self.embedding_dropout_rate)(source_embeddings)

    target_words_embeddings =  Dropout(self.embedding_dropout_rate)(target_embeddings)
